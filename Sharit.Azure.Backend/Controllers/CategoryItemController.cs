...
{
    public class CategoryItemController : TableController<CategoryItem>
    {
        protected override void Initialize(HttpControllerContext controllerContext)
        {
            base.Initialize(controllerContext);
            MobileServiceContext context = new MobileServiceContext();
            DomainManager = new EntityDomainManager<CategoryItem>(context, Request);
        }

        // GET tables/XamagramItem
        public IQueryable<CategoryItem> GetAllXamagramItem()
        {
            return Query();
        }

        // GET tables/XamagramItem/48D68C86-6EA6-4C25-AA33-223FC9A27959
        public SingleResult<CategoryItem> GetXamagramItem(string id)
        {
            return Lookup(id);
        }

        // PATCH tables/XamagramItem/48D68C86-6EA6-4C25-AA33-223FC9A27959
        public Task<CategoryItem> PatchXamagramItem(string id, Delta<CategoryItem> patch)
        {
            return UpdateAsync(id, patch);
        }

        // POST tables/XamagramItem
        public async Task<IHttpActionResult> PostXamagramItem(CategoryItem item)
        {
            ...
            templateParams["messageParam"] = item.Description + " ha sido añadido!.";
